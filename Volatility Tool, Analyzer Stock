...
class VolatilityCrushAnalyzer:
    ...
    def setup_current_straddle_section(self, parent, row):
        ...
        self.call_price_label = ttk.Label(
            pricing_frame, text="$0.00", font=("Arial", 11, "bold"), foreground="green"
        )
        ...
        self.put_price_label = ttk.Label(
            pricing_frame, text="$0.00", font=("Arial", 11, "bold"), foreground="red"
        )
        ...
        self.straddle_price_label = ttk.Label(
            pricing_frame, text="$0.00", font=("Arial", 14, "bold"), foreground="blue"
        )
    ...
    def setup_pnl_section(self, parent, row):
        ...
        self.new_straddle_label = ttk.Label(
            pnl_frame, text="$0.00", font=("Arial", 12, "bold"), foreground="blue"
        )
            ...
            if self.ib_app.connected:
                try:
                    ...
                    if server_version is not None and server_version > 0:
                        ...
                        self.update_status(
                            f"Successfully connected to IB TWS (Server: {server_version})"
                        )
                    else:
                        self.update_status(
                            "Connected but server version is not available"
                        )
                except Exception as e:
                    self.update_status(
                        f"Connection established but server version check failed: {e}"
                    )
            else:
    ...
    def fetch_market_data(self):
        ...
        self.parent.after(3000, self.process_market_data)
    ...
    def price_current_straddle(self):
        ...
        self.straddle_price_label.config(
            text=f"${straddle_price:.2f}", foreground="blue"
        )
        ...
        self.update_status(
            f"Straddle priced: ${straddle_price}, Call: ${call_price:.2f} + Put: ${put_price:.2f}"
        )
    ...
    def analyze_scenario(self):
        ...
        original_straddle_price = float(
            self.straddle_price_label.cget("text").replace("$", "")
        )

        pnl_long = new_straddle_price - original_straddle_price
        pnl_short = -pnl_long

        self.new_straddle_label.config(
            text=f"${new_straddle_price:.2f}", foreground="blue"
        )
...
class StockScout:
    def __init__(self, parent):
        self.parent = parent
        self.label = tk.Label(parent, text="Enter Stock Ticker:", font=("Arial", 12))
        self.label.pack(pady=10)
        self.entry = tk.Entry(parent, font=("Arial", 12))
        self.entry.pack(pady=5)
        self.button = tk.Button(
            parent, text="Get Stock Data", command=self.get_stock_data
        )
    ...
    def get_stock_data(self):
        ...
        daily_data = yf.download(ticker, period="ytd", interval="1d")
        if daily_data.empty:
            messagebox.showerror("Data Error", f"No daily data found for {ticker}")
            return

        intraday_data = yf.download(ticker, period="1d", interval="5m")
        if intraday_data.empty:
            messagebox.showerror("Data Error", f"No intraday data found for {ticker}")
            return

        print("*Daily YTD Data*")
...
datentime = datetime.now().strftime("[%Y-%m-%d] [%I-%M-%S %p]")
...
def main():
    ...
    scout_frame = ttk.Frame(root)
    scout_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=10, pady=10)
    StockScout(scout_frame)
